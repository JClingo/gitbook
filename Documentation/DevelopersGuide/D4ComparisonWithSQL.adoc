...
link:D4LGIntroduction-SyntacticConventions.html[Syntactic Conventions].
...
The select statement in link:D4LGComparisonWithSQL-CommonSQL.html[Common
SQL] has the following syntax:
...
link:D4LGComparisonWithSQL-CommonSQL.html[Common SQL] because it is an
...
link:D4LGComparisonWithSQL-CommonSQL.html[Common SQL] has the following
syntax:
...
link:D4LGComparisonWithSQL-CommonSQL.html[Common SQL], and from each
other.
...
*link:D4LGDataManipulation-SelectStatement.html[select]* in D4 is only
part of the statement, it is not part of the expression.
...
The *link:D4LGTableExpressions-Where.html[where]* operator in D4
...
*link:D4LGTableExpressions-OverandRemove.html[over]* and
**link:D4LGTableExpressions-OverandRemove.html[remove]**. Over allows
...
link:D4LGComparisonWithSQL-MappinganSQLQueryIntoD4-UnravelingtheColumnList.html[column
list], that of extension. In SQL, an expression is allowed to define the
value for a given column in the result.
...
The *link:D4LGTableExpressions-Add.html[add]* operator of D4 allows
...
the
link:D4LGComparisonWithSQL-MappinganSQLQueryIntoD4-UnravelingtheColumnList.html[column
list], that of renaming columns. The optional *as* keyword is used to
...
The *link:D4LGTableExpressions-Rename.html[rename]* operator of D4
...
link:D4LGComparisonWithSQL-MappinganSQLQueryIntoD4-UnravelingtheColumnList.html[column
list], that of link:D4LGTableExpressions-Aggregate.html[aggregation].
...
is relegated to the *link:D4LGTableExpressions-Add.html[add]* operator,
rather than intermixed with the
*link:D4LGTableExpressions-Aggregate.html[group]* operator. By cleanly
...
*link:D4LGTableExpressions-Where.html[where]* operator. Because D4 is an
...
link:D4LGTableExpressions-Join.html[join], suppose we are asked 'Get all
...
*link:D4LGTableExpressions-Times.html[times]* operator, and specifies
...
a variant of the *link:D4LGTableExpressions-Rename.html[rename]*
...
link:D4LGTableExpressions-OuterJoin.html[outer join]. There are three
different flavors of outer join in SQL: left, right, and full. Suppose
we are asked 'Get IDs and Names of all employees toghether with the
...
the *link:D4LGTableExpressions-OuterJoin.html[right join]* operator, but
...
link:D4LGTableExpressions-Join-NaturalJoins.html[equi-joins]. To see why
...
_link:D4LGQueryExpression.html[union-compatible]_ binary table
...
option is available for the link:D4LGTableExpressions-Union.html[union]
operator.
...
*link:D4LGTableExpressions-Intersect.html[intersect]* operator.
...
link:D4LGTableExpressions-Minus.html[minus].
...
other words, the link:D4LGOrders.html[order] by clause in D4 is not an
...
link:D4LGDataManipulation-UpdateStatement.html[update], and
link:D4LGDataManipulation-DeleteStatement.html[delete] statements in SQL
and D4 is very similar, even identical in some respects, so the mapping
is relatively straightforward. There are some important logical
differences between the semantics of the operators, particularly for the
insert and update.

First, we discuss the
link:D4LGDataManipulation-InsertStatement.html[insert] statement.
...
Note: the *link:D4LGTableExpressions-Redefine.html[redefine]* operator
...
Next, we discuss the
link:D4LGDataManipulation-UpdateStatement.html[update] statement.
...
link:D4LGImperativeStatements-Variables-AssignmentStatement.html[:=] to
indicate that this is indeed an assignment statement. The current value
